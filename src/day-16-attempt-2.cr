...
def too_much_memory_compute_all_path_pairs(node_names)
  res = node_names.permutations.flat_map do |perm|
    # include array size!
  ...
  seen = Set(Tuple(Array(String), Array(String))).new
  res.reject! do |tup|
...
def compute_all_path_pairs(node_names)
  node_names.permutations.flat_map do |perm|
    # include array size!
    (0..perm.size).map do |i|
      {perm[...i], perm[i..]}
    end
