...
def compute_paths(graph, distances, one_person_path, max_time)
  result_1, path_taken_1 = part_1_recursive(
    graph,
    distances,
    max_time: 26,
    time_elapsed: 0,
  ...
  p! result_1, path_taken_1

  smaller_graph = graph.reject(path_taken_1)

  result_2, path_taken_2 = part_1_recursive(
    smaller_graph,
    distances,
    max_time: 26,
    time_elapsed: 0,
    ...
    current_flow: 0,
    current_valve_name: "AA",
    unopened_valves: smaller_graph.select { |_k, v| v[:flow_rate] > 0 }.keys.to_set,
    path_taken: [] of String
  )
  p! result_2, path_taken_2
